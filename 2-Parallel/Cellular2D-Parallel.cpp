...
int main(int argc, char** argv)
{
	...
	for(unsigned int i = 0; i < (sizeof(all_iterations)/sizeof(*all_iterations)); ++i)
	{
		for(unsigned int j = 0; j < (sizeof(all_iterations[i])/sizeof(*all_iterations[i])); ++j)
		{
			if (all_iterations[i][j] == true)
				cout << '#';
			else
				cout << '-';	
		}
